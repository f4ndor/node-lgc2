...
const LGTV = require('./ws/weboswebsocket.js')
...
class LgTvController extends EventEmitter {
  ...
  setupLgtvInstance() {
    ...
    this.lgtv.on('connect', () => {
      if (this.connected === false) {
        ...
        this.connected = true;
        this.emit(EventNames.TV_CONNECTED);
      }
    });
    ...
    this.getTvInformation().then(() => {
      ...
      this.subscribeToServices().then(() => {
        ...
        this.connectToPointerInputSocket().then(() => {
          ...
          this.emit(EventNames.SETUP_FINISHED);
        }).catch((error) => {
  ...
  async subscribeToPowerStatusChange() {
    return this.tvSubscribe(WEBOS_URI_POWER_STATE, null, (err, res) => {
      ...
      if (tvPowerState === 'On' && this.isTvOn() === false) {
        ...
        this.emit(EventNames.SCREEN_SAVER_TURNED_ON, res);
        ...
        this.emit(EventNames.POWER_STATE_CHANGED, res);
      }
  ...
  async subscribeToAudioStatusChange() {
    return this.tvSubscribe(WEBOS_URI_AUDIO_STATUS, null, (err, res) => {
      ...
      if (statusCause) {
        // here VOLUME_UP or VOLUME_DOWN EventNames are emitted
      ...
      this.emit(EventNames.AUDIO_STATUS_CHANGED, res);
    }).catch((error) => {
      this.logDebug(error);
    });
  }
  ...
  async subscribeToForegroundAppChange() {
    return this.tvSubscribe(WEBOS_URI_FOREGROUND_APP_INFO, null, (err, res) => {
      if (res && res.appId && res.appId.length > 0) {
        ...
        this.emit(EventNames.FOREGROUND_APP_CHANGED, res);
      }
    }).catch((error) => {
      this.logDebug(error);
    });
    ...
    return this.tvSubscribe(WEBOS_URI_CURRENT_CHANNEL, null, (err, res) => {
      if (!res || err || res.errorCode) {
        ...
        if (this.getCurrentLiveTvChannelId() !== res.channelId) {
          ...
          this.emit(EventNames.LIVE_TV_CHANNEL_CHANGED, res);
        }
      }
    }).catch((error) => {
      this.currentLiveTvChannel = null;
  ...
  async subscribeToSoundOutputChange() {
    return this.tvSubscribe(WEBOS_URI_SOUND_OUPUT, null, (err, res) => {
      if (this.activeSoundOutput !== res.soundOutput) {
        ...
        this.emit(EventNames.SOUND_OUTPUT_CHANGED, res);
      }
    }).catch((error) => {
      this.logDebug(error);
    });
  ...
  async subscribeToPictureSettingsChange() {
    ...
    return this.tvSubscribe(WEBOS_URI_SYSTEM_SETTINGS, payload, (err, res) => {
      if (res && res.settings) {
        ...
        this.emit(EventNames.PICTURE_SETTINGS_CHANGED, this.pictureSettings);
      }
    }).catch((error) => {
      this.logDebug(error);
    });
  ...
  async turnOn() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.once(EventNames.TV_TURNED_ON, resolve)
    })
  }
  ...
  async turnOff() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.once(EventNames.TV_TURNED_OFF, resolve)
    })
  }
  ...
  async openLiveTv() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.once(EventNames.FOREGROUND_APP_CHANGED, resolve)
    })
  }
  ...
  webOs2xTvTurnedOn() {
    ...
    // inform listeners
    this.logInfo(`TV turned on!`);
    this.emit(EventNames.TV_TURNED_ON, {});
  }
  ...
  webOs2xTvTurnedOff() {
    ...
    // inform listeners
    this.logInfo(`TV turned off!`);
    this.emit(EventNames.TV_TURNED_OFF, {});
  }
